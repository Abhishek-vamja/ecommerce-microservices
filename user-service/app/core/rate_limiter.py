...
def rate_limit(max_requests: int, window_seconds: int, key_prefix: str = "rl") -> Callable:
    ...
    Uses IP address or authenticated identifier as the client key.
    """
    async def _rate_limiter(request: Request):
        try:
            # Determine client identifier: from header (X-Forwarded-For) or client host
            ...
            # If Redis is temporarily down, log warning but don't hard crash the service
